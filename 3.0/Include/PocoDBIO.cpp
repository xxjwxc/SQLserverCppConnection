...
PocoDBIO::PocoDBIO( void ):m_lpiDataBaseSql( NULL) 
{
	if(Init())
	{
		...
		if (m_lpiDataBaseSql->OnInit(server,db,DATABASE_TYPE::SQLSERVER,uid,pwd) != CREATE_CONNECT_SUCCESS)//创建成功
		{
...
{
	if (m_lpiDataBaseSql && SQL != "")
	{
		CString cstr( StringToWString(SQL).c_str());
		return m_lpiDataBaseSql->GetCount(cstr);
	}
	return -1;	
}
...
void PocoDBIO::GetOneData( const string &SQL, string &result )
{
	if (m_lpiDataBaseSql)
	{
		...
		m_lpiDataBaseSql->MyQuarty(cstr,_RecordSetPtr);
...
bool PocoDBIO::GetExist( const string &SQL )
{
	...
	{
		CString cstr( StringToWString(SQL).c_str());
		long err = m_lpiDataBaseSql->GetExist(cstr);
		if (err == DATA_EXIST)
			return true;
		else
			return false;
...
{
	if (m_lpiDataBaseSql && Tablename.length() > 0)
	{
		CString cstr;
		cstr.Format("SELECT * FROM %s",CString(StringToWString(Tablename).c_str()));
		return m_lpiDataBaseSql->GetColumnCount(cstr);
	}
